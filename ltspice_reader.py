...
class RawFileReader(object):
    ...
    def validate(self):
        if self.plotname.lower() != "transient analysis":
            raise InvalidRawFileError("Only transient analysis is supported.")
        elif len(self.flags) != 3 or \
            'real' not in self.flags or \
            'forward' not in self.flags or \
            'nocompression' not in self.flags:
            raise InvalidRawFileError("Flags must be real, forward, nocompression")
        elif self.nvars != len(self.vars):
            raise InvalidRawFileError("Number of variables specified does not match variables list.")
        elif self.command.lower() != "linear technology corporation ltspice iv":
            raise InvalidRawFileError("Only support raw file generated by 'Linear Technology Corporation LTspice IV'.")
...
def main():
    ...
    print "TITLE:           {}".format(rf.title)
    print "DATE:            {}".format(rf.date)
    print "PLOTNAME:        {}".format(rf.plotname)
    print "FLAGS:           {}".format(rf.flags)
    print "NVARS:           {}".format(rf.nvars)
    print "NPTS:            {}".format(rf.npts)
    print "OFFSET:          {}".format(rf.offset)
    print "COMMAND:         {}".format(rf.command)
    print "DATA START FROM: {}\n".format(rf.datapos)

    #print rf.datapos
    for v in rf.vars:
        print "{0} {1} {2}".format(v.index, v.name, v.type)
